...
//   function updateGraficosImage() {
    ...
    const cronoMob = 'lineaTiempoMob.svg';    // <- imagen Cronologia mobile
    const cronoDesk = 'lineaTiempoDesk.svg';  // <- imagen Cronologia desktop
    const cadenaMob = 'cadena-Mob.svg';    // <- imagen Cadena mobile
    const cadenaDesk = 'cadena-Desk.svg';  // <- imagen Cadena desktop
...
function createYacimientosLayer(yacJson, allPozos, allShapes) {
  ...
  yacJson.features.forEach(f => {
    ...
    layer.on("click", (e) => {
      ...
      // if (!e.classList.contains("yac-path")) {
      //   e.target.options.style.color = "#ffffff";
      //   e.target.options.style.fillColor = "#ffffff";
      // }
      // e.layer.options.color = "#000000";
      // e.layer.options.fillColor = "#000000";
      ...
      // setTimeout(() => cleanGhostPaths(), 10);
      // tempAggressiveCleaner(2000, 30);
      // document.querySelectorAll("path.leaflet-interactive").forEach(p => {
      //   if (!p.classList.contains("yac-path")) {
      //     console.log(p);
      //     p.options.style.stroke = "#ffffff";
      //   }
      // });

      map.fitBounds(layer.getBounds().pad(0.15));
      // 🔥 OCULTAR PATHS FANTASMA (los que mete Leaflet)
      // document.querySelectorAll("path.leaflet-interactive").forEach(p => {
      //   console.log('NO');
        
      //   // if (!p.classList.contains("yac-path")) {
      //   //   console.log('SI', p);
      //   //   // p.style.display = "none";   // o p.remove()
...
// --- UTIL: limpiar path fantasma y limpiar tabindex en paths ---
// Marca: los paths legítimos de yacimientos tienen la clase "yac-path" (ya los creás).
// Esta función borra paths "fantasma" (stroke/#999 + fill/#999) o paths leaflet-interactive
// que NO tengan la clase yac-path. También remueve tabindex para prevenir focus.
// function cleanGhostPaths() {
//   // 1) remover paths con stroke/fill #999 que sean obvios fantasmas
//   document.querySelectorAll('path').forEach(p => {
//     try {
//       const stroke = (p.getAttribute('stroke') || "").toLowerCase();
//       const fill = (p.getAttribute('fill') || "").toLowerCase();
//       const cls = p.classList || [];
//       // si es claramente el path fantasma
//       if ((stroke === "#999" && fill === "#999") || (stroke === "rgb(153,153,153)" && fill === "rgb(153,153,153)")) {
//         // No tocamos si es parte de un yac-path (por si acaso)
//         if (!p.classList.contains('yac-path')) p.remove();
//         return;
//       }

//       // 2) eliminar tabindex para que no vuelva a generar focus ring
//       if (p.hasAttribute('tabindex')) p.removeAttribute('tabindex');

//       // 3) si es leaflet-interactive y NO tiene clase yac-path y está con stroke/fill default gris,
//       //    bajamos su opacity para que no mezcle colores (alternativa menos destructiva)
//       if (p.classList.contains('leaflet-interactive') && !p.classList.contains('yac-path')) {
//         // si tiene stroke gris (o no tiene stroke definido), lo ocultamos suavemente
//         const s = (p.getAttribute('stroke') || "").toLowerCase();
//         if (!s || s === "#999" || s === "rgb(153,153,153)") {
//           p.style.display = 'none';
//         }
//       }
//     } catch (err) {
//       // no romper si algún nodo extraño está en la lista
//       // console.warn('cleanGhostPaths error', err);
//     }
//   });
// }

// --- Runner agresivo por tiempo corto ---
// Ejecuta la limpieza repetidamente durante X ms (para cubrir paths que se inserten después)
// function tempAggressiveCleaner(durationMs = 2000, intervalMs = 30) {
//   const start = Date.now();
//   const id = setInterval(() => {
//     cleanGhostPaths();
//     if (Date.now() - start > durationMs) clearInterval(id);
//   }, intervalMs);
//   // Ejecutar una pasada inmediata también
//   cleanGhostPaths();
//   return id;
// }

// --- Hook global: cuando se agrega cualquier layer nuevo, removemos tabindex del path si existe ---
// Esto ayuda a evitar el focus->stroke en cuanto Leaflet cree la estructura.
// map.on('layeradd', e => {
//   if (!e.layer) return;
//   // Si el layer tiene paths internos (multipolygon), quitamos tabindex
//   if (e.layer.eachLayer) {
//     e.layer.eachLayer(l => { if (l._path) { l._path.removeAttribute && l._path.removeAttribute('tabindex'); }});
//   } else if (e.layer._path) {
//     e.layer._path.removeAttribute && e.layer._path.removeAttribute('tabindex');
//   }
// });

// --- Integración con clicks en tus shapes ---
// Llamá a tempAggressiveCleaner justo después de hacer setStyle / click.
// En tu createYacimientosLayer, dentro de layer.on('click', ...) agregá estas dos líneas
// (si ya las agregaste en el click, bien; si no, ponlas dentro del click handler):
//
//    // después de layer.setStyle({...});
//    setTimeout(() => cleanGhostPaths(), 10);      // pasada inmediata corta
//    tempAggressiveCleaner(2000, 30);             // limpia repetidamente por 2s
//
// Ejemplo (fragmento):
//
// layer.on("click", () => {
//    ...your existing code...
//    layer.setStyle({ color: "#000", weight: 2, fillColor: baseColor, fillOpacity: intensity });
//    // FORZAR limpiador para cubrir el path que Leaflet pueda insertar después
//    setTimeout(() => cleanGhostPaths(), 10);
//    tempAggressiveCleaner(2000, 30);
//    showPozosForYacimiento(...);
// });
//
// Si preferís que lo haga globalmente para todos los clicks en el mapa, podés añadir:
// map.on('click', () => {
//   // pasada rápida al click global (protege clicks fuera de shapes también)
//   setTimeout(() => cleanGhostPaths(), 10);
//   // tempAggressiveCleaner(1000, 30); // 1s suficiente para la mayoría de casos
// });
